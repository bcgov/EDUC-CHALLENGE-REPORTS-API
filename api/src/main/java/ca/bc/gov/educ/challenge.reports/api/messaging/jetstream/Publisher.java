package ca.bc.gov.educ.challenge.reports.api.messaging.jetstream;
...
import ca.bc.gov.educ.challenge.reports.api.struct.v1.ChoreographedEvent;
...
import static ca.bc.gov.educ.challenge.reports.api.constants.v1.TopicsEnum.CHALLENGE_REPORTS_EVENTS_TOPIC;
...
public class Publisher {
    ...
    public static final String STREAM_NAME= "CHALLENGE_REPORTS_EVENTS";
    ...
    private void createOrUpdateSDCEventStream(final Connection natsConnection) throws IOException, JetStreamApiException {
        val streamConfiguration = StreamConfiguration.builder().name(STREAM_NAME).replicas(1).maxMessages(10000).addSubjects(CHALLENGE_REPORTS_EVENTS_TOPIC.toString()).build();
        try {
     ...
     */
    public void dispatchChoreographyEvent(final Event event, ChallengeReportsSagaEntity saga) {
        if (event != null && event.getSagaId() != null) {
            ...
            try {
                ...
                val pub = this.jetStream.publishAsync(CHALLENGE_REPORTS_EVENTS_TOPIC.toString(), JsonUtil.getJsonBytesFromObject(choreographedEvent));
