...
import static ca.bc.gov.educ.challenge.reports.api.constants.v1.EventOutcome.INITIATE_SUCCESS;
import static ca.bc.gov.educ.challenge.reports.api.constants.v1.EventOutcome.SAGA_COMPLETED;
import static ca.bc.gov.educ.challenge.reports.api.constants.v1.EventType.INITIATED;
import static ca.bc.gov.educ.challenge.reports.api.constants.v1.EventType.MARK_SAGA_COMPLETE;
import static ca.bc.gov.educ.challenge.reports.api.constants.v1.SagaStatusEnum.COMPLETED;
...
public abstract class BaseOrchestrator<T> implements EventHandler, Orchestrator {
  ...
  public BaseOrchestrator<T> end(final EventType currentEvent, final EventOutcome outcome, final SagaStep<T> stepToExecute) {
    return this.registerStepToExecute(currentEvent, outcome, (T sagaData) -> true, MARK_SAGA_COMPLETE, (Event event, ChallengeReportsSagaEntity saga, T sagaData) -> {
   ...
   */
  protected boolean isNotProcessedEvent(final EventType currentEventType, final ChallengeReportsSagaEntity saga, final Set<EventType> eventTypes) {
  ...
  protected ChallengeReportsSagaEventEntity createEventState(@NotNull final ChallengeReportsSagaEntity saga, @NotNull final EventType eventType, @NotNull final EventOutcome eventOutcome, final String eventPayload) {
    final var user = this.sagaName.length() > 32 ? this.sagaName.substring(0, 32) : this.sagaName;
    return ChallengeReportsSagaEventEntity.builder()
   ...
   */
  protected void markSagaComplete(final Event event, final ChallengeReportsSagaEntity saga, final T sagaData) {
    this.markSagaComplete(event, saga, sagaData, "");
  }
   ...
   */
  protected void markSagaComplete(final Event event, final ChallengeReportsSagaEntity saga, final T sagaData, final String payloadToSubscribers) {
    ...
    if (this.shouldSendNotificationEvent) {
      ...
      if(saga.getChallengeReportsSessionId() != null) {
        finalEvent.setChallengeReportsSessionId(saga.getChallengeReportsSessionId().toString());
      }
    ...
    final ChallengeReportsSagaEventEntity sagaEventStates = this.createEventState(saga, event.getEventType(), event.getEventOutcome(), event.getEventPayload());
   ...
   */
  private int calculateStep(final ChallengeReportsSagaEntity saga) {
  ...
  protected void postMessageToTopic(final String topicName, final Event nextEvent) {
    ...
    this.getMessagePublisher().dispatchMessage(topicName, eventString.get().getBytes());
  }
   ...
   * @return {@link ChallengeReportsSagaEventEntity} if found else null.
   */
  protected Optional<ChallengeReportsSagaEventEntity> findTheLastEventOccurred(final List<ChallengeReportsSagaEventEntity> eventStates) {
    final int step = eventStates.stream().map(ChallengeReportsSagaEventEntity::getSagaStepNumber).mapToInt(x -> x).max().orElse(0);
  ...
  public void replaySaga(final ChallengeReportsSagaEntity saga) throws IOException, InterruptedException, TimeoutException {
   ...
   * @throws IOException          if there is connectivity problem
   */
  private void replayFromLastEvent(final ChallengeReportsSagaEntity saga, final List<ChallengeReportsSagaEventEntity> eventStates, final T t) throws InterruptedException, TimeoutException, IOException {
   ...
   */
  private void findAndInvokeNextStep(final ChallengeReportsSagaEntity saga, final T t, final EventType currentEvent, final EventOutcome eventOutcome, final Event event) throws InterruptedException, TimeoutException, IOException {
   ...
   * @throws IOException          if there is connectivity problem
   */
  private void replayFromBeginning(final ChallengeReportsSagaEntity saga, final T t) throws InterruptedException, TimeoutException, IOException {
    final Event event = Event.builder()
  ...
  public void startSaga(@NotNull final ChallengeReportsSagaEntity saga) {
    try {
  ...
  @Override
  @Transactional
  public ChallengeReportsSagaEntity createSaga(@NotNull final String payload, final UUID challengeReportsSessionID, final String userName) {
   ...
   * @throws IOException          if there is connectivity problem
   */
  protected void process(@NotNull final Event event, final ChallengeReportsSagaEntity saga, final T sagaData, final SagaEventState<T> sagaEventState) throws InterruptedException, TimeoutException, IOException {
   ...
   * @throws IOException          if there is connectivity problem
   */
  protected void invokeNextEvent(final Event event, final ChallengeReportsSagaEntity saga, final T sagaData, final SagaEventState<T> sagaEventState) throws InterruptedException, TimeoutException, IOException {
